...
# https://github.com/TechStruck/TechStruck-Bot/blob/main/bot/cogs/rtfm.py
...
class RTFM(commands.Cog):
    ...
    @commands.group(invoke_without_command=True, aliases=["docs"])
    ...
    async def list_targets(self, ctx: commands.Context):
        ...
        embed = Embed(title="RTFM list of avaliable modules", color=Color.og_blurple())
        embed.description = "\n".join(
            [
                "[{0}]({1}): {2}".format(
                    target,
    ...
    @rtfm.command(name="add", hidden=True)
    @commands.is_owner()
