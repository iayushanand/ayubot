...
class Todo(commands.Cog):
    ...
    async def todo(self, ctx: commands.Context):
        ...
        """
        embed = discord.Embed(
            description="Todo based commands!",
            color=discord.Color.blurple()
        ).add_field(
            name="show",
            value="Shows your todo list",
            inline=False
        ).add_field(
            name="add",
            value="Adds a todo",
            ...
            name="strike",
            value="Strike out a todo",
            inline=False
        ).add_field(
            name="clear",
            value="Clear your todo list",
            inline=False
        )
    ...
    async def todo_show(self, ctx: commands.Context):
        ...
        if len(res)==0:
            return await ctx.reply(embed=discord.Embed(
                description=":x: Your todo list is empty",
        ...
        for i,_ in enumerate(res):
            ...
            _t = res[i].get("time")
            task = res[i].get("task")
            embed.description = embed.description + TODO_ARROW_EMOJI + f"`{unique_id}` - {task} (<t:{_t}:R>)\n"
        await ctx.reply(embed=embed)
    ...
    async def todo_add(self, ctx:commands.Context, todo:str):
        ...
        _t = int(time.time())
        task = todo
        await self.db.execute("INSERT INTO todo VALUES ($1, $2, $3, $4)", unique_id, user_id, _t, task)
        embed = discord.Embed(
    ...
    async def todo_remove(self, ctx:commands.Context, unique_id:int):
        user_id = ctx.author.id
        await self.db.execute("DELETE FROM todo WHERE unique_id = $1 AND user_id = $2", unique_id, user_id)
        embed = discord.Embed(
    ...
    async def todo_strike(self, ctx: commands.Context, unique_id:int):
        user_id = ctx.author.id
        res = await self.db.fetch("SELECT task FROM todo WHERE unique_id = $1 AND user_id = $2", unique_id, user_id)
        if len(res)==0:
            return await ctx.send(embed=discord.Embed(description=":x: todo not found",color=discord.Color.red()))
        task = res[0].get("task")
        await self.db.execute("UPDATE todo SET task = $1 WHERE unique_id = $2", f"~~{task}~~", unique_id)
        embed = discord.Embed(description=TICK_EMOJI+f"striked `{task}` from todo list",color=discord.Color.green())
        await ctx.reply(embed=embed)
