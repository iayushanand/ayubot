...
class Misc(commands.Cog):
    ...
    async def whois(self,ctx,member:discord.Member=None):
        ...
        embed = discord.Embed(colour=member.color, timestamp=datetime.utcnow(),
        ...
        embed.add_field(name="Account Created:", value=f"<t:{int(datetime.timestamp(member.created_at))}:R>",inline=True)
        embed.add_field(name="Server Joined:", value=f"<t:{int(datetime.timestamp(member.joined_at))}:R>",inline=True)
    ...
    async def serverinfo(self,ctx):
        ...
        embed = discord.Embed(colour=guild.me.color, timestamp=datetime.utcnow(),
        ...
        embed.add_field(name="Created:", value=f"{guild.created_at.strftime('%d/%m/%y')}",inline=True)
        embed.add_field(name="Verification Level:", value=f'{guild.verification_level}',inline=True)
        embed.add_field(name="Explicit Content Filter:", value=f'{guild.explicit_content_filter}',inline=True)
        embed.add_field(name="AFK Channel:", value=f'{guild.afk_channel.mention}',inline=True)
        embed.add_field(name="AFK Timeout:", value=f'{guild.afk_timeout}',inline=True)
        embed.add_field(name="System Channel:", value=f'{guild.system_channel.mention}',inline=True)
    ...
    async def joined(self,ctx: commands.Context, rank: int):
        ...
        embed = discord.Embed(title = f"The {ord(rank)} person to join is: ", description=all_members[rank - 1].mention)
        await ctx.reply(embed=embed)
    ...
    async def calc(self,ctx, *, equation:str):
        try:
            await ctx.reply(embed=discord.Embed(description=f"Result: {eval(equation)}", color=0x00ff00))
        except Exception as e:
            await ctx.reply(embed=discord.Embed(description=f"Error: {e}",color=0xFF0000))
