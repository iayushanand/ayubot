...
from ext.consts import LOG_CHANNEL_ID, LEAVE_MESSAGE_CHANNEL_ID
...
class Logs(commands.Cog):
    ...
    async def on_member_remove(self, member):
        ...
        await self.log.send(embed=embed)
        embed = discord.Embed(
            description=f"**{member}** left the server. Hope they will not get there dinner today <:Bruh:811480176868327437>", color=discord.Color.dark_theme())
    ...
    async def on_member_update(self, before, after: discord.Member):
        ...
        if before.nick != after.nick:
            embed = discord.Embed(
                ...
                url=before.display_avatar
            ).set_footer(
                text="id: {0}".format(before.id)
            ).add_field(
                name="Before",
                value="{0}".format(before.nick)
            ).add_field(
                name="After",
                value=f"{after.nick}"
            )
    ...
    async def on_message_edit(self, before, after):
        ...
        if before.content != after.content:
            embed = discord.Embed(
                ...
                text="id: {0}".format(before.author.id)
            ).add_field(
                name="Before",
