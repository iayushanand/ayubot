...
class Levelling(commands.Cog):
    ...
    async def _level_set(
        self, ctx: commands.Context, member: discord.Member, level: int
    ):
        try:
            await self.db.execute(
                "UPDATE level SET level = $1, xp = 0 WHERE user_id = $2",
                level,
                member.id,
            )
        except:
            await ctx.reply(
                embed=discord.Embed(
                    description=":x: Something went wrong!", color=discord.Color.red()
                )
            )
            raise Exception
        await ctx.reply(
            embed=discord.Embed(
                ...
                + f"updated level for {member.mention} set it to {level}",
                color=discord.Color.green(),
            )
        )
