...
class UserHelpModal(Modal):
    ...
    async def on_submit(self, interaction: Interaction) -> None:
        embed = (
            discord.Embed(color=discord.Color.random(), timestamp=datetime.utcnow())
            .set_author(
                name=interaction.user.name, icon_url=interaction.user.avatar.url
            )
            .add_field(name="Language:", value=self.children[0].value, inline=False)
            .add_field(name="Topic:", value=self.children[1].value, inline=False)
            .add_field(name="Description:", value=self.children[2].value, inline=False)
        )
        await interaction.response.send_message(
            "Join thread to get help!", ephemeral=True
        )
        ...
        thread = await msg.create_thread(name=self.children[1].value)
